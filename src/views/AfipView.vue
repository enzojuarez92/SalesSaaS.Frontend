...
const documentTitle = (invoice: Invoice) => ({ 1: "FACTURA A", 6: "FACTURA B", 11: "FACTURA C", 3: "NOTA DE CRÉDITO A", 8: "NOTA DE CRÉDITO B", 13: "NOTA DE CRÉDITO C" })[invoice.afipVoucherType || 0] || "TICKET";
...
function printInvoice(invoice: Invoice) { const business = getPrintBusiness(auth.tenantId); const fiscalState = invoice.cae ? "COMPROBANTE ELECTRÓNICO" : invoice.afipResult === "Internal" ? "DOCUMENTO NO FISCAL" : "PENDIENTE DE ARCA"; if (!printReceipt({ tenantId: auth.tenantId, printFormat: tenant.printFormat, businessName: business.name || tenant.businessName, businessTaxId: business.taxId || tenant.businessTaxId, businessLegalName: business.legalName || tenant.businessLegalName, businessTaxCondition: business.taxCondition || tenant.businessTaxCondition, businessAddress: business.address || tenant.businessAddress, businessPhone: business.phone || tenant.businessPhone, businessLogoUrl: business.logoUrl || tenant.businessLogoUrl, receiptNumber: invoice.number, date: invoice.issuedAtUtc, customer: invoice.customerName, paymentMethod: "Según comprobante", total: invoice.totalAmount, items: [{ product: "Comprobante fiscal", quantity: 1, unitPrice: invoice.totalAmount, subtotal: invoice.totalAmount }], documentTitle: documentTitle(invoice), fiscalLabel: fiscalState, cae: invoice.cae || undefined, caeExpirationDate: invoice.caeExpirationDate, verificationUrl: invoice.barCode })) error.value = "El navegador bloqueó la ventana de impresión."; }
async function download(invoice: Invoice) { try { const { data } = await api.get(`/invoices/${invoice.id}/pdf`, { params: { tenantId: auth.tenantId }, responseType: "blob" }); const url = URL.createObjectURL(new Blob([data], { type: "application/pdf" })); const link = document.createElement("a"); link.href = url; link.download = `${invoice.number}.pdf`; link.click(); window.setTimeout(() => URL.revokeObjectURL(url), 60_000); } catch (cause) { error.value = apiError(cause); } }
