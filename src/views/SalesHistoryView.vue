...
const loading = ref(false), detailLoading = ref(false), paymentMethod = ref<number | "">("");
...
const params = () => ({ tenantId: auth.tenantId, warehouseId: tenant.activeWarehouseId || undefined, paymentMethod: paymentMethod.value || undefined });
...
<template>
  ...
  <section class="panel">
    <div class="history-toolbar"><label>Medio de pago<select v-model.number="paymentMethod" @change="load"><option value="">Todos</option><option v-for="option in paymentOptions" :key="option.value" :value="option.value">{{ option.label }}</option></select></label></div>
...
<style scoped>
.history-toolbar{display:flex;margin-bottom:1rem}.history-toolbar label{min-width:220px}.payment-summary{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:.65rem;margin:0 0 1rem}.payment-summary article{display:grid;gap:.2rem;padding:.75rem;border:1px solid #e2e8f0;border-radius:.7rem;background:#f8fafc}.payment-summary small{color:#64748b}.history-actions{display:flex;gap:.4rem}.sale-detail{width:min(100%,720px)}.sale-meta{display:grid;grid-template-columns:repeat(3,1fr);gap:.6rem;margin:1rem 0}.sale-meta div{padding:.65rem;border-radius:.6rem;background:#f8fafc}.sale-meta dt,.detail-table small{display:block;color:#64748b;font-size:.75rem}.sale-meta dd{margin:.2rem 0 0;font-weight:600}.detail-table{width:100%;border-collapse:collapse}.detail-table th,.detail-table td{padding:.6rem;border-bottom:1px solid #e2e8f0;text-align:left}.detail-table th:last-child,.detail-table td:last-child{text-align:right}.detail-total{display:flex;justify-content:space-between;margin:1rem 0;font-size:1.1rem}.modal-actions{display:flex;justify-content:flex-end;gap:.6rem}@media(max-width:650px){.sale-meta{grid-template-columns:1fr}.detail-table{font-size:.8rem}}
</style>
