...
async function confirmReceipt() {
  if (!receiptOrder.value || receiptLines.value.some(item => item.quantity < 0 || item.unitCost < 0)) {
    notify("Revisá las cantidades y costos recibidos.", true);
    return;
  }
  await run(async () => {
    await api.post(`/purchases/orders/${receiptOrder.value!.id}/receive`, {
      ...
      items: receiptLines.value.map(item => ({ productId: item.productId, receivedQuantity: item.quantity, unitCost: item.unitCost })),
    });
...
<template>
  ...
  <div v-if="receiptOrder" class="modal-backdrop"><section class="modal receipt-modal"><button class="icon-button modal-close" aria-label="Cerrar" @click="receiptOrder = null"><X /></button><h2>Confirmar recepción de mercadería</h2><p><strong>{{ supplierName(receiptOrder.supplierId) }}</strong><br>Revisá las cantidades y los costos reales antes de impactar el stock.</p><form novalidate @submit.prevent="confirmReceipt"><div class="receipt-grid"><table><thead><tr><th>Producto</th><th>Pedido</th><th>Cantidad recibida</th><th>Costo unitario real</th><th>Subtotal</th></tr></thead><tbody><tr v-for="(line, index) in receiptLines" :key="line.productId"><td>{{ line.product }}<small>{{ line.sku }}</small></td><td>{{ receiptOrder.items[index]?.quantity }}</td><td><input v-model.number="line.quantity" type="number" min="0" /></td><td><CurrencyInput v-model="line.unitCost" :min="0" /></td><td>{{ money(line.quantity * line.unitCost) }}</td></tr></tbody></table></div><div class="order-footer"><div><span>Total recibido</span><strong>{{ money(receiptTotal) }}</strong></div><div class="modal-actions"><button type="button" class="secondary" @click="receiptOrder = null">Cancelar</button><button class="primary" :disabled="busy"><PackageCheck :size="16" />Confirmar y registrar recepción</button></div></div></form></section></div>
