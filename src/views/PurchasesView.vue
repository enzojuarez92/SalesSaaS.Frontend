...
import { Ban, Check, Eye, FilePlus2, LoaderCircle, Minus, PackageCheck, Paperclip, Pencil, Plus, Printer, ReceiptText, Save, ScrollText, Trash2, X } from "lucide-vue-next";
...
type ReceiptLine = { productId: string; product: string; sku: string; quantity: number; unitCost: number };
...
const receiptOrder = ref<PurchaseDetail | null>(null);
...
function adjustLineQuantity(index: number, change: number) {
  const line = lines.value[index];
  if (!line) return;
  if (line.quantity + change <= 0) removeLine(index);
  else line.quantity += change;
}
...
async function openReceipt(order: Purchase) {
  try {
    const detail = (await api.get<PurchaseDetail>(`/purchases/orders/${order.id}`)).data;
    receiptOrder.value = detail;
    receiptLines.value = detail.items.map(item => ({ productId: item.productId, product: item.product, sku: item.sku, quantity: item.quantity, unitCost: item.unitCost }));
  } catch (cause) { notify(apiError(cause), true); }
}
async function confirmReceipt() {
  if (!receiptOrder.value || receiptLines.value.some(item => item.quantity < 0)) {
    notify("Revisá las cantidades recibidas.", true);
    return;
  }
  await run(async () => {
    await api.post(`/purchases/orders/${receiptOrder.value!.id}/receive`, {
      tenantId: auth.tenantId,
      purchaseOrderId: receiptOrder.value!.id,
      items: receiptLines.value.map(item => ({ productId: item.productId, receivedQuantity: item.quantity })),
    });
...
<style scoped>
.tabs{display:flex;flex-wrap:wrap;gap:.5rem;margin:1rem 0}.tabs button{padding:.6rem 1rem;border:1px solid #e2e8f0;border-radius:.6rem;background:#fff}.tabs button.active{background:#ec4899;color:#fff;border-color:#ec4899}.order-actions,.invoice-actions{display:flex;align-items:center;flex-wrap:wrap;gap:.45rem}.order-actions .secondary{padding:.45rem .65rem;font-size:.8rem}.danger-action{color:#be123c}.purchase-modal,.receipt-modal{width:min(100%,60rem);max-height:calc(100dvh - 24px);overflow:auto}.supplier-picker{display:flex;align-items:end;gap:.75rem}.supplier-picker label{flex:1}.entry-row{display:grid;grid-template-columns:2fr .8fr 1fr auto;gap:.7rem;align-items:end;margin:1.25rem 0}.add-item{height:2.9rem;white-space:nowrap}.item-grid,.receipt-grid{overflow-x:auto;border:1px solid #e2e8f0;border-radius:.8rem}.item-grid table,.receipt-grid table{width:100%;border-collapse:collapse}.item-grid th,.item-grid td,.receipt-grid th,.receipt-grid td{padding:.7rem;text-align:left;border-bottom:1px solid #e2e8f0}.item-grid th:nth-child(n+2),.item-grid td:nth-child(n+2),.receipt-grid th:nth-child(n+2),.receipt-grid td:nth-child(n+2){text-align:right}.item-grid th:last-child,.item-grid td:last-child{text-align:center}.item-grid tr:last-child td,.receipt-grid tr:last-child td{border-bottom:0}.item-grid small,.receipt-grid small,.detail-table small,.responsive-table small{display:block;color:#64748b;margin-top:.2rem}.quantity-stepper{display:inline-flex;align-items:center;gap:.45rem}.quantity-stepper button{display:grid;place-items:center;width:1.75rem;height:1.75rem;border:1px solid #dbe3ef;border-radius:.45rem;background:#fff;color:#e91e63}.quantity-stepper strong{min-width:1.5rem;text-align:center}.receipt-grid input{max-width:6.5rem;text-align:right}.receipt-unit-cost{display:inline-block;font-weight:650;font-variant-numeric:tabular-nums;color:#334155}.order-footer{display:flex;justify-content:space-between;align-items:center;margin-top:1rem;padding:1rem;background:#fdf2f8;border-radius:.75rem}.order-footer>div{display:grid;gap:.15rem;color:#64748b}.order-footer strong{font-size:1.35rem;color:#be185d}.icon-upload{display:grid;place-items:center;width:2.25rem;height:2.25rem;border:1px solid #dbe3ef;border-radius:.7rem;color:#ec4899;cursor:pointer}.icon-upload input{display:none}.modal-actions{display:flex;justify-content:flex-end;align-items:center;gap:.65rem;margin-top:1rem}.supplier-modal,.detail-modal{width:min(100%,42rem)}.detail-table{width:100%;border-collapse:collapse;margin:1rem 0}.detail-table th,.detail-table td{padding:.65rem;border-bottom:1px solid #e2e8f0;text-align:left}.detail-table th:nth-child(n+2),.detail-table td:nth-child(n+2){text-align:right}.detail-total{display:flex;justify-content:space-between;margin:1rem 0;font-size:1.1rem}.supplier-active{display:flex;align-items:center;gap:.75rem;min-height:4.5rem;padding:.8rem 1rem;margin:.8rem 0;border:1px solid #e2e8f0;border-radius:.8rem;cursor:pointer}.supplier-active input{width:1.15rem;height:1.15rem;margin:0;accent-color:#ec4899}.supplier-active span{display:grid;gap:.2rem}.supplier-active small{color:#64748b}.wide-field{grid-column:span 2}.statement-line{display:flex;justify-content:space-between;gap:1rem;padding:.8rem 0;border-bottom:1px solid #e2e8f0}@media(max-width:760px){.entry-row{grid-template-columns:1fr 1fr}.entry-row>label:first-child{grid-column:1/-1}.add-item{width:100%}.supplier-picker{align-items:stretch;flex-direction:column}.wide-field{grid-column:span 1}.order-footer{align-items:stretch;flex-direction:column}.order-footer .primary{width:100%}.modal-actions{flex-wrap:wrap}}
</style>
