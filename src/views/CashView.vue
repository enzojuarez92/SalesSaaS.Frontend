...
<template>
  ...
  <div v-if="showOpen" class="modal-backdrop" @click.self="showOpen = false"><section class="modal"><button class="icon-button modal-close" @click="showOpen = false"><X /></button><div class="modal-icon"><DoorOpen /></div><h2>Abrir caja</h2><p>Ingresá el fondo inicial en efectivo para {{ tenant.warehouses.find(w => w.id === tenant.activeWarehouseId)?.name || "el depósito seleccionado" }}.</p><form @submit.prevent="submitOpen"><label>Fondo inicial<input v-model.number="openingBalance" type="number" min="0" step="0.01" autofocus /></label><button class="primary full" :disabled="saving"><LoaderCircle v-if="saving" class="spin" :size="16" />Abrir turno</button></form></section></div>
  <div v-if="showMovement" class="modal-backdrop" @click.self="showMovement = false"><section class="modal"><button class="icon-button modal-close" @click="showMovement = false"><X /></button><div class="modal-icon"><component :is="movementIncome ? ArrowUpCircle : ArrowDownCircle" /></div><h2>{{ movementIncome ? "Registrar ingreso" : "Registrar egreso" }}</h2><form @submit.prevent="submitMovement"><label>Importe<input v-model.number="movementAmount" type="number" min="0.01" step="0.01" autofocus /></label><label>Medio de pago<select v-model.number="movementPayment"><option :value="1">Efectivo</option><option :value="4">Transferencia</option><option :value="5">Mercado Pago</option></select></label><label>Motivo<input v-model.trim="movementDescription" maxlength="300" placeholder="Ej. Retiro para cambio" /></label><button class="primary full" :disabled="saving">Guardar movimiento</button></form></section></div>
  <div v-if="showClose && current" class="modal-backdrop" @click.self="showClose = false"><section class="modal"><button class="icon-button modal-close" @click="showClose = false"><X /></button><div class="modal-icon"><LockKeyhole /></div><h2>Arqueo y cierre</h2><p>El sistema espera <strong>{{ money(current.expectedCash) }}</strong> en efectivo.</p><form @submit.prevent="submitClose"><label>Importe físico contado<input v-model.number="closingBalance" type="number" min="0" step="0.01" autofocus /></label><p class="difference" :class="{ mismatch: physicalDifference !== 0 }"><CircleAlert :size="17" />{{ physicalDifference === 0 ? "Sin diferencia" : `${physicalDifference > 0 ? "Sobrante" : "Faltante"}: ${money(Math.abs(physicalDifference))}` }}</p><button class="primary full" :disabled="saving">Confirmar cierre</button></form></section></div>
</template>
