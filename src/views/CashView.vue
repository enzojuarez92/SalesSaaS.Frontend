...
<template>
  ...
  <div v-if="showOpen" class="modal-backdrop" @click.self="showOpen = false"><section class="modal"><button class="icon-button modal-close" @click="showOpen = false"><X /></button><div class="modal-icon"><DoorOpen /></div><h2>Abrir caja</h2><p>Ingresá el fondo inicial en efectivo para {{ tenant.warehouses.find(w => w.id === tenant.activeWarehouseId)?.name || "el depósito seleccionado" }}.</p><form novalidate @submit.prevent="submitOpen"><label>Fondo inicial<CurrencyInput v-model="openingBalance" :min="0" autofocus /></label><button class="primary full" :disabled="saving"><LoaderCircle v-if="saving" class="spin" :size="16" />Abrir turno</button></form></section></div>
  <div v-if="showMovement" class="modal-backdrop" @click.self="showMovement = false"><section class="modal"><button class="icon-button modal-close" @click="showMovement = false"><X /></button><div class="modal-icon"><component :is="movementIncome ? ArrowUpCircle : ArrowDownCircle" /></div><h2>{{ movementIncome ? "Registrar ingreso" : "Registrar egreso" }}</h2><form novalidate @submit.prevent="submitMovement"><label>Importe<CurrencyInput v-model="movementAmount" :min="0.01" autofocus /></label><label>Medio de pago<select v-model.number="movementPayment"><option :value="1">Efectivo</option><option :value="4">Transferencia</option><option :value="5">Mercado Pago</option></select></label><label>Motivo<input v-model.trim="movementDescription" maxlength="300" placeholder="Ej. Retiro para cambio" /></label><button class="primary full" :disabled="saving">Guardar movimiento</button></form></section></div>
