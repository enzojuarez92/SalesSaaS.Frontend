...
async function load() {
  ...
  try {
    await tenant.load();
    const [businessResponse, usersResponse] = await Promise.all([api.get("/settings/business", { params: { tenantId: auth.tenantId } }), api.get<TenantUser[]>("/users", { params: { tenantId: auth.tenantId } })]);
    Object.assign(business, businessResponse.data); users.value = usersResponse.data;
...
<style scoped>
.tabs{display:flex;gap:.5rem;margin-bottom:1rem}.tabs button{padding:.6rem 1rem;border:1px solid #e2e8f0;border-radius:.6rem;background:#fff}.tabs button.active{color:#fff;background:#ec4899;border-color:#ec4899}.wide{grid-column:span 2}.warehouse-list{display:grid;gap:.5rem;border:1px solid #e2e8f0;border-radius:.75rem;padding:.8rem}.check-row{display:flex;align-items:center;gap:.55rem;cursor:pointer}.check-row input{width:auto}@media(max-width:600px){.wide{grid-column:span 1}}
</style>
