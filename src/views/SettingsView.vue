...
import { Building2, ChevronRight, CircleHelp, Download, ExternalLink, ImageUp, KeyRound, MapPin, PlayCircle, Plus, ShieldCheck, Trash2, TriangleAlert, X } from "lucide-vue-next";
...
async function generateCsr() {
  if (!afip.issuerTaxId || !afip.certificateAlias) { error.value = "Completá el CUIT emisor y el Alias antes de generar la solicitud CSR."; return; }
  generatingCsr.value = true; error.value = "";
  try {
    const { data } = await api.post<{ csrPem: string; privateKeyPem: string }>("/settings/afip-csr", { tenantId: auth.tenantId, issuerTaxId: afip.issuerTaxId, certificateAlias: afip.certificateAlias });
    afip.privateKeyContent = data.privateKeyPem;
    downloadText(data.csrPem, `${afip.certificateAlias}.csr`);
    notify("Solicitud CSR descargada. La clave privada quedó cargada en el formulario.");
  } catch (cause) { error.value = apiError(cause); } finally { generatingCsr.value = false; }
}
...
<template>
  ...
  <section v-if="tab === 'afip'" class="panel">
    ...
    <details class="afip-guide">
      <summary><span class="guide-summary-icon"><ShieldCheck :size="19" /></span><span><strong>Guía para activar Factura Electrónica</strong><small>Video y pasos para configurar tu certificado ARCA.</small></span><ChevronRight class="guide-chevron" :size="18" /></summary>
      <div class="afip-guide-content">
        <div class="afip-video-card">
          <div><span class="eyebrow"><PlayCircle :size="15" /> VIDEO TUTORIAL</span><h3>Cómo generar y vincular el certificado</h3><p>Un instructivo visual original de KloverCloud para completar el proceso de ARCA y Web Services.</p><a class="secondary guide-link" href="/tutorials/klovercloud-arca-tutorial.mp4" target="_blank" rel="noopener noreferrer"><PlayCircle :size="16" />Abrir video<ExternalLink :size="14" /></a></div>
          <video controls preload="metadata" aria-label="Tutorial KloverCloud para generar certificados ARCA"><source src="/tutorials/klovercloud-arca-tutorial.mp4" type="video/mp4" />Tu navegador no puede reproducir este video.</video>
        </div>
        <ol class="arca-steps">
          <li><span>1</span><div><h3>Punto de Venta Web Services</h3><p>Ingresá con Clave Fiscal a ARCA, abrí <strong>Facturación y Registración</strong> y creá un punto de venta de tipo <strong>Factura Electrónica - Web Services</strong>.</p><a href="https://www.arca.gob.ar/fe/documentos/AccionesarealizarparaconsumirunWebservicedeFacturaElectr.pdf" target="_blank" rel="noopener noreferrer">Ver guía oficial <ExternalLink :size="13" /></a></div></li>
          <li><span>2</span><div><h3>Descargar solicitud <code>.csr</code></h3><p>Completá primero el CUIT y Alias de abajo. KloverCloud genera la solicitud y prepara la clave privada para que no tengas que usar herramientas técnicas.</p><button type="button" class="secondary guide-action" :disabled="generatingCsr" @click="generateCsr"><Download :size="16" />{{ generatingCsr ? 'Generando solicitud…' : 'Generar y descargar CSR' }}</button></div></li>
          <li><span>3</span><div><h3>Crear certificado digital en ARCA</h3><p>En <strong>Administración de Certificados Digitales</strong>, subí el archivo <code>.csr</code> descargado y guardá el certificado <code>.crt</code> que emite ARCA.</p><a href="https://www.arca.gob.ar/ws/documentacion/certificados.asp" target="_blank" rel="noopener noreferrer">Abrir documentación de certificados <ExternalLink :size="13" /></a></div></li>
          <li><span>4</span><div><h3>Asociar el servicio WSFE</h3><p>Desde <strong>Administrador de Relaciones de Clave Fiscal</strong>, asociá el Alias al servicio <strong>Facturación Electrónica</strong>.</p><a href="https://www.arca.gob.ar/ws/documentacion/certificados.asp" target="_blank" rel="noopener noreferrer">Ver cómo asociar el servicio <ExternalLink :size="13" /></a></div></li>
          <li><span>5</span><div><h3>Cargar en KloverCloud</h3><p>Pegá el contenido del certificado <code>.crt</code> y verificá que la clave privada <code>.key</code> esté cargada antes de guardar.</p></div></li>
        </ol>
      </div>
    </details>
    ...
    <form class="form-grid afip-configuration-grid" novalidate @submit.prevent="saveAfip">
...
<style scoped>
.tabs{display:flex;flex-wrap:wrap;gap:.5rem;margin-bottom:1rem}.tabs button{padding:.6rem 1rem;border:1px solid #e2e8f0;border-radius:.6rem;background:#fff}.tabs button.active{color:#fff;background:#ec4899;border-color:#ec4899}.wide{grid-column:span 2}.warehouse-list{display:grid;gap:.5rem;border:1px solid #e2e8f0;border-radius:.75rem;padding:.8rem}.check-row{display:flex;align-items:center;gap:.55rem;cursor:pointer}.check-row input{width:auto}.success-text{display:block;margin-top:.3rem;color:#047857}.logo-control{display:flex;justify-content:space-between;align-items:center;gap:1rem;border:1px dashed #cbd5e1;border-radius:.75rem;padding:.85rem}.logo-control strong,.logo-control small{display:block}.logo-actions{display:flex;align-items:center;gap:.5rem}.logo-actions img{width:3.5rem;height:3.5rem;object-fit:contain;border:1px solid #e2e8f0;border-radius:.5rem;background:#fff}.upload-logo{position:relative;display:inline-flex;align-items:center;gap:.4rem;cursor:pointer}.upload-logo input{position:absolute;width:1px;height:1px;opacity:0;pointer-events:none}.danger-button{color:#e11d48}.afip-guide{margin:1.1rem 0;padding:.9rem 1rem;border:1px solid #f9a8d4;border-radius:.8rem;background:#fdf2f8}.afip-guide summary{display:flex;align-items:center;gap:.5rem;color:#be185d;font-weight:700;cursor:pointer;list-style:none}.afip-guide summary::-webkit-details-marker{display:none}.afip-guide-content{max-width:52rem;margin-top:.85rem;color:#475569}.afip-guide-content p{margin:0}.afip-guide-content ol{display:grid;gap:.6rem;margin:.8rem 0 0;padding-left:1.3rem}.afip-guide-content li{padding-left:.15rem;line-height:1.5}.afip-guide-content code{padding:.08rem .28rem;border-radius:.25rem;background:#fce7f3;color:#9d174d}.field-help{display:flex;align-items:flex-start;gap:.35rem;color:#475569;font-size:.75rem;font-weight:400;line-height:1.4;min-height:2.4rem}.field-help svg{flex:0 0 auto;margin-top:.04rem;color:#db2777}.afip-configuration-grid>label{align-content:start}.afip-configuration-grid>label:not(.wide){grid-template-rows:auto 54px 2.4rem}.afip-configuration-grid>label:not(.wide)>input,.afip-configuration-grid>label:not(.wide)>select{height:54px}.dark .afip-guide{border-color:#7c2d5b;background:#2e1930}.dark .afip-guide summary{color:#f9a8d4}.dark .afip-guide-content{color:#d1dced}.dark .afip-guide-content code{background:#442039;color:#fbcfe8}.dark .field-help{color:#b8c7dc}.dark .field-help svg{color:#f9a8d4}@media(max-width:600px){.wide{grid-column:span 1}.logo-control{align-items:flex-start;flex-direction:column}.afip-configuration-grid>label:not(.wide){grid-template-rows:auto 54px auto}}
.afip-guide{display:block;margin:1.1rem 0;border:1px solid #f9a8d4;border-radius:1rem;background:linear-gradient(115deg,#fffafd,#fff);overflow:hidden}.afip-guide summary{display:flex;align-items:center;gap:.75rem;padding:1rem 1.1rem;color:#334155;cursor:pointer;list-style:none}.afip-guide summary::-webkit-details-marker{display:none}.guide-summary-icon{display:grid;place-items:center;width:2.25rem;height:2.25rem;border-radius:.7rem;background:#fce7f3;color:#be185d}.afip-guide summary span:nth-child(2){display:grid;gap:.12rem;flex:1}.afip-guide summary strong{font-size:.95rem}.afip-guide summary small{color:#64748b;font-size:.78rem;font-weight:400}.guide-chevron{color:#be185d;transition:transform .2s ease}.afip-guide[open] .guide-chevron{transform:rotate(90deg)}.afip-guide[open] summary{border-bottom:1px solid #fce7f3}.afip-guide-content{display:grid;gap:1rem;padding:1.1rem;color:#475569}.afip-video-card{display:grid;grid-template-columns:minmax(0,1fr) minmax(260px,.8fr);gap:1rem;align-items:center;padding:1rem;border:1px solid #f5d0e6;border-radius:.85rem;background:#fff}.afip-video-card>div{display:grid;gap:.5rem}.afip-video-card h3,.arca-steps h3{margin:0;color:#1e293b}.afip-video-card p,.arca-steps p{margin:0;line-height:1.5}.eyebrow{display:flex;align-items:center;gap:.35rem;color:#be185d;font-size:.7rem;font-weight:800;letter-spacing:.1em}.guide-link,.guide-action{width:max-content;display:inline-flex;align-items:center;gap:.4rem;margin-top:.25rem}.afip-video-card video{width:100%;aspect-ratio:16/9;border:0;border-radius:.6rem;background:#1e293b}.arca-steps{display:grid;gap:.7rem;margin:0;padding:0;list-style:none}.arca-steps li{display:grid;grid-template-columns:2rem minmax(0,1fr);gap:.75rem;padding:.85rem;border:1px solid #e2e8f0;border-radius:.8rem;background:#fff}.arca-steps>li>span{display:grid;place-items:center;width:2rem;height:2rem;border-radius:999px;background:#fce7f3;color:#be185d;font-size:.82rem;font-weight:800}.arca-steps li>div{display:grid;gap:.35rem}.arca-steps h3{font-size:.9rem}.arca-steps code{padding:.08rem .28rem;border-radius:.25rem;background:#fce7f3;color:#9d174d}.arca-steps a{display:inline-flex;align-items:center;gap:.3rem;width:max-content;color:#be185d;font-size:.8rem;font-weight:700;text-decoration:none}.arca-steps a:hover{text-decoration:underline}.field-help{display:flex;align-items:flex-start;gap:.35rem;color:#475569;font-size:.75rem;font-weight:400;line-height:1.4;min-height:2.4rem}.field-help svg{flex:0 0 auto;margin-top:.04rem;color:#db2777}.afip-configuration-grid>label{align-content:start}.afip-configuration-grid>label:not(.wide){grid-template-rows:auto 54px 2.4rem}.afip-configuration-grid>label:not(.wide)>input,.afip-configuration-grid>label:not(.wide)>select{height:54px}.afip-test-mode{display:flex;align-items:flex-start;gap:.45rem;margin:.15rem 0 1rem;padding:.62rem .75rem;border:1px solid #fde68a;border-radius:.65rem;background:#fffbeb;color:#92400e;font-size:.8rem;line-height:1.4}.afip-test-mode svg{flex:0 0 auto;margin-top:.03rem}.dark .afip-guide{border-color:#7c2d5b;background:linear-gradient(115deg,#1c1d31,#211b32)}.dark .afip-guide[open] summary{border-color:#55304e}.dark .afip-guide summary{color:#f8fafc}.dark .afip-guide summary small,.dark .afip-video-card p,.dark .arca-steps p{color:#cbd5e1}.dark .guide-summary-icon,.dark .arca-steps>li>span{background:#442039;color:#f9a8d4}.dark .afip-video-card,.dark .arca-steps li{border-color:#475a77;background:#18233a}.dark .afip-video-card h3,.dark .arca-steps h3{color:#f8fafc}.dark .arca-steps code{background:#442039;color:#fbcfe8}.dark .field-help{color:#b8c7dc}.dark .field-help svg{color:#f9a8d4}.dark .afip-test-mode{border-color:#785a13;background:#2d2715;color:#fde68a}@media(max-width:680px){.afip-video-card{grid-template-columns:1fr}.guide-link,.guide-action{width:100%;justify-content:center}.wide{grid-column:span 1}.logo-control{align-items:flex-start;flex-direction:column}.afip-configuration-grid>label:not(.wide){grid-template-rows:auto 54px auto}}
</style>
