...
const usersError = ref(""), mercadoPagoError = ref("");
...
const mercadoPago = reactive({ publicKey: "", accessToken: "", webhookSecret: "", hasAccessToken: false, hasWebhookSecret: false, updatedAtUtc: "" });
...
async function load() {
  ...
  try {
    const [businessResponse, usersResponse, mercadoPagoResponse] = await Promise.allSettled([
      api.get("/settings/business", { params: { tenantId: auth.tenantId } }),
      api.get<TenantUser[]>("/users", { params: { tenantId: auth.tenantId } }),
      api.get("/settings/mercadopago", { params: { tenantId: auth.tenantId } }),
    ]);
    ...
    mercadoPagoError.value = mercadoPagoResponse.status === "rejected" ? apiError(mercadoPagoResponse.reason) : "";
    if (mercadoPagoResponse.status === "fulfilled") Object.assign(mercadoPago, mercadoPagoResponse.value.data, { accessToken: "", webhookSecret: "" });
...
async function saveMercadoPago() { saving.value = true; error.value = ""; try { const { data } = await api.put("/settings/mercadopago", { tenantId: auth.tenantId, publicKey: mercadoPago.publicKey, accessToken: mercadoPago.accessToken || null, webhookSecret: mercadoPago.webhookSecret || null }); Object.assign(mercadoPago, data, { accessToken: "", webhookSecret: "" }); success.value = "Credenciales de Mercado Pago guardadas."; } catch (cause) { error.value = apiError(cause); } finally { saving.value = false; } }
...
<template>
  ...
  <div class="tabs"><button type="button" :class="{ active: tab === 'business' }" @click="selectTab('business')">Empresa</button><button type="button" :class="{ active: tab === 'afip' }" @click="selectTab('afip')">ARCA</button><button type="button" :class="{ active: tab === 'mercadopago' }" @click="selectTab('mercadopago')">Mercado Pago</button><button type="button" :class="{ active: tab === 'users' }" @click="selectTab('users')">Usuarios</button></div>
  ...
  <section v-if="tab === 'mercadopago'" class="panel"><h2>Mercado Pago</h2><p class="muted">Las claves se guardan cifradas y sólo se usan para crear el checkout de suscripción de este negocio.</p><p v-if="mercadoPagoError" class="error" role="alert">{{ mercadoPagoError }}</p><form class="form-grid" novalidate @submit.prevent="saveMercadoPago"><label class="wide">Public Key<input v-model.trim="mercadoPago.publicKey" maxlength="300" placeholder="APP_USR-..." /></label><label>Access Token<input v-model="mercadoPago.accessToken" type="password" autocomplete="new-password" placeholder="Dejá vacío para conservar el actual" /><small v-if="mercadoPago.hasAccessToken" class="success-text">Access Token configurado</small></label><label>Secret de Webhooks<input v-model="mercadoPago.webhookSecret" type="password" autocomplete="new-password" placeholder="Dejá vacío para conservar el actual" /><small v-if="mercadoPago.hasWebhookSecret" class="success-text">Secret configurado</small></label><button class="primary full" :disabled="saving">{{ saving ? 'Guardando…' : 'Guardar credenciales' }}</button></form></section>
